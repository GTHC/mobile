...
const teams = (state = initalState, action) => {
  switch (action.type) {
    ...
    case 'END_GET_TEAMS': {
      // this formats the teams into data that can be read by the Dropdown
      // component in the TeamSignUp component
      const teamDropDownOptions = action.payload.data.map(team => {
        // having a serperate color variable rather than just
        // sayingteam.tent_type avoids warnings although it would it still work
